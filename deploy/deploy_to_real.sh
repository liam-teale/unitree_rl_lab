...
#
# Does, in order:
#   1. Put the wired NIC on the robot's 192.168.123.x subnet (removing any
#      conflicting routable IPv4 address that would break DDS multicast).
...
# Drop other global-scope IPv4 addresses on this NIC (e.g. a DHCP 10.x) that
# would otherwise hijack DDS multicast routing. Keep the target + link-local.
if [ "$KEEP_OTHER" -eq 0 ]; then
    while read -r other; do
        ...
        say "Removing conflicting address $other from $IFACE"
        sudo ip addr del "$other" dev "$IFACE"
    done < <(ip -o -4 addr show dev "$IFACE" scope global | awk '{print $4}')
fi

# Add the host address if not already present.
